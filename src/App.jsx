...
function resolveBillSpendIds(bill,accounts,debts){
  ...
  const bid=resolveBankAccountIdForExpense(pf,bill?.bankAccountId,accounts);
...
function SpendingView({expenses,setExpenses,budgetGoals,setBGoals,categories,setEditItem,onAdd,showToast,showUndoToast,household,applySpend,applyRefund}){
...
}function BillsView({bills,setBills,setEditItem,onAdd,showToast,showUndoToast,household,requestNotifPermission,applySpend,applyRefund,accounts,debts}){
  ...
  return(
    <div style={{flex:1,minHeight:0,width:"100%",maxWidth:640,margin:"0 auto",background:darkMode?C.navy:C.bg,fontFamily:IF,display:"flex",flexDirection:"column",position:"relative",overflow:"hidden",boxSizing:"border-box",height:"100%",maxHeight:"100dvh"}}>
      ...
      <div id="fv-scroll" style={{flex:1,minHeight:0,minWidth:0,width:"100%",overflowY:"auto",overflowX:"hidden",WebkitOverflowScrolling:"touch",padding:"16px max(16px, env(safe-area-inset-left)) max(110px, calc(88px + env(safe-area-inset-bottom))) max(16px, env(safe-area-inset-right))",boxSizing:"border-box"}}>
            ...
            <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4}}>
              {(()=>{
                ...
                const chips=[
                  ...
                  ...urgentPayBills.map(b=>({l:"✓ Pay "+b.name,a:()=>{const ba=parseFloat(b.amount)||0;const r=resolveBillSpendIds(b,accounts,debts);if(!r.ok){showToast&&showToast(r.msg,"error");return;}if(ba)applySpend(normalizePaidFrom(b.paidFrom),ba,r.cid,r.bid);setBills(p=>p.map(x=>x.id===b.id?{...x,paid:true}:x));}})),
                ];
        ...
        {tab==="bills"&&<BillsView bills={bills} setBills={setBills} setEditItem={setEditItem} onAdd={()=>om("bill")} showToast={showToast} showUndoToast={showUndoToast} household={household} requestNotifPermission={requestNotifPermission} applySpend={applySpend} applyRefund={applyRefund} accounts={accounts} debts={debts}/>}
        {tab==="more"&&!isMoreTab&&(
          <div className="fu">
