...
/** Per-debt chart color: saved `debt.color` or stable slot from `debts` order. */
function debtDisplayColor(debt,debts){
...
/** Principal portion of a loan payment (payment minus static monthly interest on balance before pay). */
function principalFromLoanPayment(loanBalance,rateApr,payment){
  const bal=parseFloat(loanBalance)||0;
  const pay=parseFloat(payment)||0;
  if(bal<=0||pay<=0)return 0;
  const r=(parseFloat(rateApr)||0)/100/12;
  const interest=bal*r;
  return round2(Math.min(bal,Math.max(0,pay-interest)));
}
...
/** Sub-account id for checking/savings spend/refund: explicit id, else settings default / first row (same as pickers + submit()). Optional settings required when multiple sub-accounts exist and explicit may be empty. */
function resolveBankAccountIdForExpense(paidFrom,explicitId,accounts,settings){
  const pf=normalizePaidFrom(paidFrom);
  ...
  if(pf==="checking"){
    ...
    if(eid&&ch.length&&!ch.some(c=>String(c.id)===String(eid)))eid=settings?pickDefaultBankAccountId("checking",accounts,settings)||"":"";// stale id → default
    return eid;
  }
  if(pf==="savings"){
    ...
    if(eid&&sv.length&&!sv.some(c=>String(c.id)===String(eid)))eid=settings?pickDefaultBankAccountId("savings",accounts,settings)||"":"";
    return eid;
  }
  return"";
}
/** creditDebtId + bankAccountId for marking a bill paid / unpaying — matches expense rules */
function resolveBillSpendIds(bill,accounts,debts,settings){
  ...
  if(pf==="credit"){
    ...
    if(cards.length===1)cid=String(cards[0].id);
      ...
      const id=bill?.creditDebtId?String(bill.creditDebtId):"";
      if(!id||!cards.some(c=>String(c.id)===id))return{ok:false,msg:"Open Edit on this bill and choose which card it pays from.",cid:undefined,bid:undefined};
      cid=id;
    }
  }
  const bid=resolveBankAccountIdForExpense(pf,bill?.bankAccountId,accounts,settings);
  if(pf==="checking"&&ch.length>=2&&!bid)return{ok:false,msg:"Edit the bill and choose which checking account.",cid,bid:undefined};
  if(pf==="savings"&&sv.length>=2&&!bid)return{ok:false,msg:"Edit the bill and choose which savings account.",cid,bid:undefined};
...
function chatMatchBill(bills,phrase){
...
/** Expense date from natural language + optional ISO in message. */
function chatPickExpenseDate(text){
  const t=text.toLowerCase();
  const dt=new Date();
  const _localDs=d2=>d2.getFullYear()+"-"+String(d2.getMonth()+1).padStart(2,"0")+"-"+String(d2.getDate()).padStart(2,"0");
  ...
  if(lastDayM){const target=DAYS.indexOf(lastDayM[1].toLowerCase());const d=new Date(dt);const curr=d.getDay();const diff=curr>=target?curr-target:curr-target+7;d.setDate(d.getDate()-Math.max(diff,1));return _localDs(d);}
  return todayStr();
}
...
function parseMsg(text,categories,debts,bills,opts={}){
  ...
  if(bills&&bills.length){
    let hit=null;
    const tryPhrase=ph=>{const b=chatMatchBill(bills,ph);if(b&&!b.paid)hit=b;};
    let m=text.match(/^\s*(?:mark\s+)?(.+?)\s+as\s+paid\s*\.?\s*$/i);if(m)tryPhrase(m[1]);
    if(!hit){m=text.match(/^\s*paid\s+(?:my\s+|the\s+)?(.+?)\s*\.?\s*$/i);if(m&&!/^\d+$/.test(m[1].trim()))tryPhrase(m[1]);}
    if(!hit){m=text.match(/^\s*(.+?)\s+paid\s*\.?\s*$/i);if(m&&!/^\d/.test(m[1].trim())&&m[1].trim().length>1)tryPhrase(m[1]);}
  ...
  const dt=new Date();
  let date=chatPickExpenseDate(text);
  ...
  if(amount){
    ...
    if((t.includes("primary income")||t.includes("salary set")||t.includes("income set")||/\bpaycheck\b/.test(t)||/\bsalary\b/.test(t)||/\bprimary\b/.test(t)||/\btake[\s-]?home\b/.test(t)||/\bper pay\b/.test(t))&&!/\b(other|freelance|rental|dividend|trading income)\b/.test(t))return{type:"income",key:"primary",amount};
  }
  ...
  if(amount){
    ...
    let name=text.replace(/\$?[\d,]+(?:\.\d{1,2})?/g,"").replace(/^\s*(?:add\s+)?(?:an?\s+)?(?:expense\s*:?\s*|log\s+|logged\s+|spent\s+|bought\s+|paid\s+)/gi,"").replace(/\b(for|on|the|a|my|at|to|today|yesterday|tomorrow)\b/gi,"").replace(/\s+/g," ").trim();
...
function SpendingView({expenses,setExpenses,budgetGoals,setBGoals,categories,setEditItem,onAdd,showToast,showUndoToast,household,applySpend,applyRefund,accounts={},settings={}}){
        ...
        return(
          <div key={b.id} style={{marginBottom:8}}>
            <div className="rw" style={{display:"flex",alignItems:"center",gap:12,padding:"14px 14px",background:C.surface,border:`1.5px solid ${b.paid?C.border:d<0?C.redMid:d<=7?C.amberMid:C.border}`,borderRadius:14}}>
              <button onClick={()=>{const x=b;const wasPaid=x.paid;const nowPaid=!wasPaid;const bamt=parseFloat(x.amount)||0;const bpf=normalizePaidFrom(x.paidFrom);let pd=0;if(nowPaid&&!wasPaid){const r=resolveBillSpendIds(x,accounts,debts,settings);if(!r.ok){showToast&&showToast(r.msg,"error");return;}if(x.linkedDebtId){const debt=debts.find(d=>String(d.id)===String(x.linkedDebtId));if(debt&&isLoanDebt(debt))pd=principalFromLoanPayment(debt.balance,debt.rate,bamt);}if(applySpend&&bamt)applySpend(bpf,bamt,r.cid,r.bid);if(pd>0)setDebts(p=>p.map(d=>String(d.id)===String(x.linkedDebtId)?{...d,balance:String(round2(Math.max(0,parseFloat(d.balance||0)-pd)))}:d));}else if(!nowPaid&&wasPaid){const r=resolveBillSpendIds(x,accounts,debts,settings);if(applyRefund&&bamt)applyRefund(bpf,bamt,r.cid,r.bid);const addBack=parseFloat(x.loanPrincipalApplied)||0;if(addBack>0&&x.linkedDebtId)setDebts(p=>p.map(d=>String(d.id)===String(x.linkedDebtId)?{...d,balance:String(round2(parseFloat(d.balance||0)+addBack))}:d));}if(nowPaid){setTimeout(()=>showToast&&showToast("✓ Paid — "+x.name),0);try{navigator.vibrate&&navigator.vibrate([30,10,30]);}catch{}}setBills(p=>p.map(xx=>{if(xx.id!==x.id)return xx;if(nowPaid&&xx.recurring&&xx.recurring!=="One-time"){return{...xx,paid:false,dueDate:(()=>{const d=new Date((xx.dueDate||todayStr())+"T00:00:00");if(xx.recurring==="Weekly"){d.setDate(d.getDate()+7);}else if(xx.recurring==="Bi-weekly"){d.setDate(d.getDate()+14);}else if(xx.recurring==="Quarterly"){d.setMonth(d.getMonth()+3);}else if(xx.recurring==="Annual"){d.setFullYear(d.getFullYear()+1);}else{d.setMonth(d.getMonth()+1);}return d.getFullYear()+"-"+String(d.getMonth()+1).padStart(2,"0")+"-"+String(d.getDate()).padStart(2,"0");})(),paidDate:todayStr(),loanPrincipalApplied:undefined};}if(nowPaid&&!wasPaid)return{...xx,paid:true,...(pd>0?{loanPrincipalApplied:pd}:{loanPrincipalApplied:undefined})};if(!nowPaid&&wasPaid)return{...xx,paid:false,loanPrincipalApplied:undefined};return xx;}));}} style={{background:"none",border:"none",cursor:"pointer",color:b.paid?C.green:C.border,padding:0,display:"flex",flexShrink:0}}>{b.paid?<CheckCircle2 size={22}/>:<Circle size={22}/>}</button>
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:12,marginTop:2,display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                  ...
                  {b.recurring&&b.recurring!=="One-time"&&<span style={{color:C.textLight}}>{b.recurring}</span>}{b.linkedDebtId&&<span style={{color:C.accent,fontSize:11,fontWeight:600}}> · loan payment</span>}{b.notes&&<span style={{color:C.textFaint,fontSize:11}}>· {b.notes}</span>}
    ...
    <div className="fu">
      ...
      {debts.length>0&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}><div style={{background:C.redBg,borderRadius:12,padding:"10px 12px",textAlign:"center"}}><div style={{fontSize:10,color:C.red,marginBottom:2,fontWeight:600,letterSpacing:.5}}>REMAINING</div><div style={{fontFamily:MF,fontWeight:800,fontSize:14,color:C.red}}>{fmt(totalDebt)}</div></div><div style={{background:C.greenBg,borderRadius:12,padding:"10px 12px",textAlign:"center"}}><div style={{fontSize:10,color:C.green,marginBottom:2,fontWeight:600,letterSpacing:.5}}>PAID DOWN</div><div style={{fontFamily:MF,fontWeight:800,fontSize:14,color:C.green}}>{fmt(totalPaidDown)}</div></div><div style={{background:C.accentBg,borderRadius:12,padding:"10px 12px",textAlign:"center"}}><div style={{fontSize:10,color:C.accent,marginBottom:2,fontWeight:600,letterSpacing:.5}}>PROGRESS</div><div style={{fontFamily:MF,fontWeight:800,fontSize:14,color:C.accent}}>{overallPct}%</div></div></div>}
        <div style={{display:"flex",gap:8}}>
          ...
          <button className="ba" onClick={()=>(onAddDebt||(()=>setModal("debt")))()} style={{display:"flex",alignItems:"center",gap:5,background:C.accent,border:"none",borderRadius:10,padding:"8px 14px",color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer"}}><Plus size={13}/>Add Debt</button>
        </div>
      </div>
...
function AppInner(){
  ...
  function submit(){
    if(modal==="expense"){
      ...
      setDebts(p=>[...p,{id:_debtId,name:form.name,balance:form.balance,original:form.original||form.balance,rate:form.rate||"",minPayment:form.minPayment||"",type:form.type||"",debtKind:_dk,color:(form.color&&isValidHexColor(form.color)?form.color.trim():DEBT_PALETTE[p.length%DEBT_PALETTE.length])}]);
      if(_addBill){
  ...
  return(
    ...
    <div style={{flex:1,minHeight:0,width:"100%",maxWidth:640,margin:"0 auto",background:darkMode?C.navy:C.bg,fontFamily:IF,display:"flex",flexDirection:"column",position:"relative",overflow:"hidden",boxSizing:"border-box",height:"100%",maxHeight:"100dvh"}}>
            ...
            <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4}}>
              {(()=>{
                ...
                const chips=[
                  ...
                  ...urgentPayBills.map(b=>({l:"✓ Pay "+b.name,a:()=>{const ba=parseFloat(b.amount)||0;const r=resolveBillSpendIds(b,accounts,debts,settings);if(!r.ok){showToast&&showToast(r.msg,"error");return;}if(ba)applySpend(normalizePaidFrom(b.paidFrom),ba,r.cid,r.bid);setBills(p=>p.map(x=>x.id===b.id?{...x,paid:true}:x));}})),
                ];
      ...
      {editItem&&editItem.type==="debt"&&<EditModal item={editItem} categories={categories} debts={debts} accounts={accounts} settings={settings} onSave={u=>{setDebts(p=>p.map(x=>x.id===editItem.data.id?{...x,...u}:x));const mp=parseFloat(u.minPayment||0);if(mp>0)setBills(p=>p.map(b=>String(b.linkedDebtId)!==String(editItem.data.id)?b:{...b,amount:String(mp)}));showToast("✓ Debt updated");setEditItem(null);}} onDelete={()=>setConfirm({title:"Delete Debt",message:`Delete "${editItem.data.name}"?`,onConfirm:()=>{const did=editItem.data.id;setDebts(p=>p.filter(x=>x.id!==did));setBills(p=>p.filter(x=>String(x.linkedDebtId)!==String(did)));setEditItem(null);setConfirm(null);},danger:true})} onClose={()=>setEditItem(null)}/>}
      {modal==="quickactions"&&(()=>{
