...
function pickDefaultCreditDebtId(settings,debts){
...
function resolveBillSpendIds(bill,accounts,debts,settings){
  ...
  if(pf==="credit"){
    ...
    if(cards.length===1)cid=String(cards[0].id);
      ...
      if(!id||!cards.some(c=>String(c.id)===id))return{ok:false,msg:"Open Edit on this bill and choose which card it pays from, or set a default card in Settings \u2192 Defaults.",cid:undefined,bid:undefined};
      cid=id;
    }
  }
...
budget · trades · ytd · recent expenses · payday · help"}]);
  ...
  useEffect(()=>{
    if(pending?.type==="expense"){
      ...
      if(pf==="credit"&&cards.length&&(!cid||!cards.some(c=>String(c.id)===cid)))cid=pickDefaultCreditDebtId(settings,debts)||"";
...
function RecurringView({expenses,setExpenses,categories,showToast,appReady,recurrings,setRecurrings,applySpend,defaultExpensePaidFrom,accounts={},settings={},debts=[]}){
  ...
  useEffect(()=>{
    ...
    if(newExps.length){
      ...
      if(dpf==="checking"&&ch.length===1&&!bankAccountId)bankAccountId=String(ch[0].id);
      if(dpf==="savings"&&sv.length===1&&!bankAccountId)bankAccountId=String(sv[0].id);
      let creditOk=true,cashOk=true;
      if(dpf==="credit"&&tot>0){if(!cards.length)creditOk=false;else if(!creditDebtId)creditOk=false;}
      if(dpf==="checking"&&tot>0&&ch.length>=2&&!bankAccountId)cashOk=false;
      if(dpf==="savings"&&tot>0&&sv.length>=2&&!bankAccountId)cashOk=false;
      if(!creditOk||!cashOk){
        recurCommit=recurrings;
        if(showToast)showToast(!creditOk?"Recurring skipped: add cards under Debt or set a default credit card in Settings \u2192 Defaults.":"Recurring skipped: set default checking or savings in Settings \u2192 Defaults.","error");
      }else{
