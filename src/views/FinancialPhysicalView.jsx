...
import { AlertCircle, Shield, CreditCard, TrendingDown, PiggyBank, Zap } from "lucide-react";
...
export default function FinancialPhysicalView({income,expenses,debts,accounts,bills,savingsGoals}){
  ...
  const priorities=[];
  const PRIORITY_ICONS=[AlertCircle,Shield,CreditCard,TrendingDown,PiggyBank,Zap];
  if(ov)priorities.push({Icon:AlertCircle,c:C.red,t:"Pay overdue bills",d:ov+" bill"+(ov!==1?"s":" ")+"past due"});
  if(liq<1000)priorities.push({Icon:Shield,c:C.amber,t:"Build $1,000 emergency fund",d:"Have "+fmt(liq)+" — need "+fmt(Math.max(0,1000-liq))+" more"});
  if(dti>36)priorities.push({Icon:CreditCard,c:C.red,t:"Reduce debt load",d:"DTI "+dti.toFixed(1)+"% — target under 28%"});
  ...
  return(<div className="fu">
    ...
    {priorities.length>0&&<div style={{marginBottom:16}}><div style={{fontFamily:MF,fontWeight:700,fontSize:14,color:C.text,marginBottom:10}}>Top priorities</div>{priorities.slice(0,3).map((p,i)=>{const Icon=p.Icon||PRIORITY_ICONS[i%PRIORITY_ICONS.length];return(<div key={i} className="fv-card" style={{padding:14,marginBottom:8,borderLeft:`4px solid ${p.c}`}}><div style={{display:"flex",gap:10,alignItems:"flex-start"}}><div style={{width:36,height:36,borderRadius:10,background:p.c+"14",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Icon size={18} color={p.c} strokeWidth={2}/></div><div><div style={{fontSize:13,fontWeight:700,color:C.text}}>{p.t}</div><div style={{fontSize:12,color:C.textMid,marginTop:3,lineHeight:1.5}}>{p.d}</div></div></div></div>);})}</div>}
    <div className="fv-card" style={{padding:18}}>
